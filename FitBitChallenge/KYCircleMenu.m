...
- (void)viewDidLoad {
  ...
  [self.view addSubview:menu_];
    ...
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(callRecognizer:)];
    [longPress setMinimumPressDuration:1.f];
    longPress.delegate = self;
    
    UITapGestureRecognizer *tapPress = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(runButtonActions:)];
...
- (void)callRecognizer: (UILongPressGestureRecognizer *) sender {
    //[fbchallengeViewController makeActionSheet];
    if (sender.state == UIGestureRecognizerStateEnded) {
         [[NSNotificationCenter defaultCenter] postNotificationName:@"zhongqil.FitBitChallenge.makeActionSheet" object:self];
         NSLog(@"recognize called");
    }
}
